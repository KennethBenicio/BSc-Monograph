...
for jj = 1:length(SNR)
    ...
    for mc = 1:MCC
        mc
        %% CASE 1 P = 1  
        [ADR_propos_c1(jj,mc),ADR_no_IRS_c1(jj,mc),~,~,~,] = tcc_pilot_sig_beam_opt_general(sys_par_c1,4,SNR(jj),SNR_TR);
        
        %% CASE 2 P = 2
        [ADR_propos_c2(jj,mc),ADR_no_IRS_c2(jj,mc),~,~,~,] = tcc_pilot_sig_beam_opt_general(sys_par_c2,4,SNR(jj),SNR_TR);
        
        %% CASE 3 P = 3
        [ADR_propos_c3(jj,mc),ADR_no_IRS_c3(jj,mc),~,~,~,] = tcc_pilot_sig_beam_opt_general(sys_par_c3,4,SNR(jj),SNR_TR);
        
        %% CASE 4 P = 4
        [ADR_propos_c4(jj,mc),ADR_no_IRS_c4(jj,mc),~,~,~,] = tcc_pilot_sig_beam_opt_general(sys_par_c4,4,SNR(jj),SNR_TR);   
    end
end
toc
