%% tcc 2021 -- x Pilot process function

function [ADR_prop,ADR_no_IRS,W_opt,Q_opt,S_opt] = tcc_pilot_sig_beam_opt_general(sys_par,QAM_pilot,SNR,SNR_TR)
...
%------- Known matrix
C     = zeros(Mr,Mr*Mt*N,K);  
WW     = dftmtx(K+1)(:,1:end-1) - dftmtx(K+1)(:,2:end);
WW     = WW(1:N,:); % N x K

for kk = 1:K
    F_kk = kron(x_pilots(kk,:), eye(Mr));
...
tenC_K_2 = tens2mat(C,2); %MrK x MrMtN

%---------------------------------x Loop x --------------------------------
for p =1:P
    %% ------- Pilot processing
    zp  = pinv(tenC_K_2) * tenY_p_3(:,p); % MrMtN x1 approx vec(kr(tenH(:,:,p).',tenG(:,:,p)))) = vec kron(B_tx(:,p),A_rx(:,p) ) * kr(B_irs(:,p).' , A_irs(:,p)).' ).'
...
for p=1:P
    IRS_gain_hat(p)        = rp(:,p).'*WW_opt(:,p); % With est
    IRS_gain(p)            = A_irs(:,p).'*diag(WW_opt(:,p))*B_irs(:,p); % True channels
end

%%  Precoders, Combiners and ADR/
...
%% ADR(Without pathloss estimation)
Heq         = W_opt'*A_rx * diag(sc_ab) * diag(IRS_gain) * B_tx.'*Q_opt; % True channels
