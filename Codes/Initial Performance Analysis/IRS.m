function [NMSE,Capacity] = IRS(snr_db,P,N)
    Mt = 2;
    Mr = 2;
    K = N*Mr;
...
%-----------------------PARTE 1: Gerando sinal Y---------------------------%
    ...
    for pp = 1:P
        ...
        [w_p, _, q_p] = svd(reshape(U_p, [Mr, Mt]),'econ');
        W{(pp)} = w_p(:,1);
        Q{(pp)} = q_p(:,1);
    ...
    Hefc = conj(reshape(cell2mat(W), [P Mr]))*Arx_matrices*alpha*Hefc*beta*Btx_matrices.'*reshape(cell2mat(Q),[Mt P])*X;
    Capacity = abs(log2(det(eye(P) + (Hefc*(conj(Hefc)'))./(P*variance_noise))));
endfunction
